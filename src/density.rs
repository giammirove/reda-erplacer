    ...
    T: Numeric + std::fmt::Debug,
{
    ...
    thread_maps
        ...
        .for_each(|(thread_index, thread_buf)| {
            ...
            for i in start..end {
                ...
                let bx_l = ((xi - xl) * inv_bin_size_x)
                    .floor()
                    .max(zero)
                    .to_usize()
                    .unwrap();
                let bx_h = (((xi - xl + sxi) * inv_bin_size_x)
                    .ceil()
                    .to_usize()
                    .unwrap()
                    + 1)
                ...
                let by_l = ((yi - yl) * inv_bin_size_y)
                    .floor()
                    .max(zero)
                    .to_usize()
                    .unwrap();
                let by_h = (((yi - yl + syi) * inv_bin_size_y)
                    .ceil()
                    .to_usize()
                    .unwrap()
                    + 1)
    ...
    buf_map.par_iter_mut().for_each(|v| {
        *v = (*v).min(bin_size_x * bin_size_y);
    });
}
...
pub(crate) fn compute_density_overflow_map_parallel<T>(
    ...
    T: Numeric,
{
    let map_size = num_bins_x * num_bins_y;
    let num_threads = rayon::current_num_threads();

    // 2MB per thread budget — use thread-local buffers when they fit,
    // otherwise fall back to atomic Mutex-per-bin
    // Always use thread-local buffers for deterministic results
    let use_thread_local = true;
    ...
    if use_thread_local {
        let chunk_size = x.len().div_ceil(num_threads);
        let thread_results: Vec<Vec<T>> = (0..num_threads)
            .into_par_iter()
            .map(|tid| {
                let mut local_map = vec![T::zero(); map_size];
                let start = tid * chunk_size;
                let end = ((tid + 1) * chunk_size).min(x.len());
                for i in start..end {
                    ...
                    let mut bx_l =
                        unsafe { ((xi - xl) * inv_bin_size_x).to_isize().unwrap_unchecked() };
                    let mut bx_h = unsafe {
                    ...
                    let mut by_l =
                        unsafe { ((yi - yl) * inv_bin_size_y).to_isize().unwrap_unchecked() };
                    let mut by_h = unsafe {
                    ...
                    for bx in bx_l as usize..bx_h as usize {
                        ...
                        for by in by_l as usize..by_h as usize {
                            ...
                            unsafe { *local_map.get_unchecked_mut(row + by) += px * py };
                        }
                    }
                }
                local_map
        ...
        density_map
            ...
            .enumerate()
            .for_each(|(i, dest)| {
                let mut sum = unsafe { *initial_density_map.get_unchecked(i) };
                for buf in &thread_results {
                    sum = sum + unsafe { *buf.get_unchecked(i) };
                }
                *dest = sum;
            });
    } else {
        // Large grid: use per-bin Mutex to avoid excessive memory
        (0..x.len()).into_par_iter().for_each(|i| {
            ...
            for bx in bx_l as usize..bx_h as usize {
                ...
                for by in by_l as usize..by_h as usize {
                    let py = triangle_density_function(yi, syi, yl, by, bin_size_y);
                    let idx = row + by;
                    unsafe { *density_map_mutex.get_unchecked(idx).lock() += px * py };
                }
            }
        });
